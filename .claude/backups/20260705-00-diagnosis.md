...
**現況證據**：2026-07-05 盤點時，本 repo 沒有 CLAUDE.md、沒有任何教訓紀錄檔；`list_triggers` 回傳空。`index.html` 的 meta 描述寫「每週五自動從 TrainingPeaks YouTube 頻道擷取」，但排程實際上不存在——過去的更新都是人工開新 session，每次從零重新理解專案。
...
**現況證據**：`index.html` 331 行 / 約 26KB，每週資料（影片卡片）、CSS、版面結構全部混在同一個檔案。
